...
# ===========================================================================
	...
	def __init__(self, address=__INA219_ADDRESS, debug=False):
		self.i2c = rpi_i2c(address, debug=debug, name="ina219")
		self.address = address
		self.debug = debug
		
		self.ina219_currentDivider_mA = 10
		self.ina219_powerDivider_mW   = 2
...
#		self.ina219SetCalibration_32V_2A()
	...
	def cal_device(self,device=0):
		if (device == 1):
			...
			print "***ERROR!! need to specify which INA219 board you are using... look at the back"
			bytes = [0xA0, 0x00]
		self.i2c.writeList(self.__INA219_REG_CALIBRATION, bytes)
	...
	def config_device(self):
		...
		bytes = [(config >> 8) & 0xFF, config & 0xFF]
		self.i2c.writeList(self.__INA219_REG_CONFIG, bytes)
#		self.i2c.readList(self.__INA219_REG_CONFIG)
...
#		result = self.i2c.readU16(self.__INA219_REG_BUSVOLTAGE)
		result = self.i2c.readList(self.__INA219_REG_BUSVOLTAGE,length=2)
		value = result[0]*256 + result[1]
		# Shift to the right 3 to drop CNVR and OVF and multiply by LSB
		return (value >> 3) * 4
		
	def getShuntVoltage_raw(self):
		result = self.i2c.readList(self.__INA219_REG_SHUNTVOLTAGE,length=2)
		if (result[0] >> 7 == 1):
	...
	def getCurrent_raw(self):
		result = self.i2c.readList(self.__INA219_REG_CURRENT,length=2)
		if (result[0] >> 7 == 1):
			testint = (result[0]*256 + result[1])
			othernew = self.twosToInt(testint, 16)
			return othernew
	...
	def getPower_raw(self):
		result = self.i2c.readList(self.__INA219_REG_POWER,length=2)
		if (result[0] >> 7 == 1):
			testint = (result[0]*256 + result[1])
			othernew = self.twosToInt(testint, 16)
			return othernew
